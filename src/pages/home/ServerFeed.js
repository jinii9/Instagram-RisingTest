...
import DemoImg1 from "../../assets/images/ghibli.jpg";
import DemoImg2 from "../../assets/images/penguin.jpg";
import DemoProfileImg from "../../assets/images/Profile.png";

// 슬라이드
function Slide ({IMGdata}) {
    console.log("-----슬라이드이미지:",IMGdata);
    // let slideItem = 
    // 슬라이드 셋팅
    const settings = {
        className: "",
...
// 인스타 피드 컴포넌트
const ServerFeed = ({data}) => {
    console.log("FeedData------", data.image[0].imgUrl);

    // console.log("FeedData", data.image.imgUrl);

    // 임시 사진
    // <img alt="LoginMainImg" src={DemoImg1}/>
    const DemoImg = [];
    if(data){
        DemoImg.push(DemoImg1);
        DemoImg.push(DemoImg2);


    }
    // console.log("임시확인", DemoImg1);
    ...
    return (
        <Wrap>
            <FeedWrap>
                <Header>
                    <ProfileWrap>
                        <ProfileImg>
                            ...
                            <Link to={""}><img alt='작성자의 프로필 사진' src={DemoImg2}/></Link>
                        </ProfileImg>
                        <WriterId>
                            {data.user.id}
                        </WriterId>
                ...
                <Section>
                    {/* 슬라이드 */}
                    <Slide IMGdata={DemoImg}/>
                    {/* {(data.image).map((item) => (
                        <Slide IMGdata={item}/>
                    ))} */}
                    {/* <Slide IMGdata={data.image}/> */}
                ...
                <Footer>
                    <IconWrap>
                        <ThreeWrap>
                            ...
                            <IconBtn type='button'>
                                <Link to={`/${data.post.postUuid}`} state={{data}} onClick={openModal}>
                                    <Message/>
                                </Link>
                            </IconBtn>
                    ...
                    <GoodWrap>좋아요 {data.countLike}개</GoodWrap>
                    <PostWrap>
                        <WritingWrap>
                            <Writer>{data.user.id}</Writer>
                        ...
                        {/* <WritingWrap Comment> */}
                            ...
                            {/* {data.comment.length<3 ? (
                                ...
                                <>
                                    <WritingWrap>
                                        <CommentAll>댓글 {data.commentCount}개 모두 보기</CommentAll>
                                    </WritingWrap>
                                    <WritingWrap Comment>
